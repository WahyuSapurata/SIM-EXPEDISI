                    ...
                    <div class="card shadow-sm">
                        <div class="card-body">
                            <div class="d-flex align-items-center gap-5">
                                ...
                                <div>
                                    <div class="fs-6">Pendapatan</div>
                                    <div class="fs-5" id="total-pendapatan">Rp 0</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card shadow-sm">
                        <div class="card-body">
                            <div class="d-flex align-items-center gap-5">
                                ...
                                <div>
                                    <div class="fs-6">Laba Bersih</div>
                                    <div class="fs-5" id="total-laba">Rp 0</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                            <h6 class="m-0 font-weight-bold text-primary">Total Pendapatan dan Pengeluaran</h6>
                        </div>
                        <!-- Card Body -->
                        <div class="card-body">
...
@section('script')
    <script>
        ...
        $(document).ready(function() {
            ...
            // Menanggapi perubahan pada elemen select tahun
            $('#filter_year_select').on('change', function() {
                $('#noData').addClass('d-none');
                const selectedYear = $(this).val();
                if (selectedYear) {
                    $('#filter_month_select').val(null).trigger('change'); // Kosongkan pilihan bulan
                    fetchData(selectedYear,
                        null); // Panggil fetchData dengan tahun yang dipilih dan bulan null
                }
            });

            // Menanggapi perubahan pada elemen select bulan
            $('#filter_month_select').on('change', function() {
                $('#noData').addClass('d-none');
                const selectedMonth = $(this).val();
                if (selectedMonth) {
                    $('#filter_year_select').val(null).trigger('change'); // Kosongkan pilihan tahun
                    fetchData(null,
                        selectedMonth); // Panggil fetchData dengan bulan yang dipilih dan tahun null
                }
            });
            ...
            function getGrafik(data) {
                ...
                $.each(data.pendapatan, function(x, y) {
                    total_pendapatan += y;
                })
                $.each(data.pengeluaran, function(x, y) {
                    total_pengeluaran += y;
                })
                $.each(data.laba, function(x, y) {
                    total_laba += y;
                })
                $('#total-pendapatan').html('Rp ' + numeral(total_pendapatan).format('0,0'));
                $('#total-pengeluaran').html('Rp ' + numeral(total_pengeluaran).format('0,0'));
                $('#total-laba').html('Rp ' + numeral(total_laba).format('0,0'));
                // Update chart data
                if (myChart) {
                    myChart.destroy();
                }
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            },
                            {
                                label: 'Laba',
                                data: data.laba,
                                backgroundColor: 'rgb(0, 255, 0)', // Hijau menyala
                                borderColor: 'rgb(0, 255, 0, 1)',
                                borderWidth: 1,
                                barPercentage: 0.6, // Lebar batang
                                categoryPercentage: 0.8 // Persentase kategori untuk lebar grup
                            }
                        ]
                    },
                    options: {
            ...
            function clearGrafik() {
                ...
                $('#total-pendapatan').html('Rp 0');
                $('#total-pengeluaran').html('Rp 0');
                $('#total-laba').html('Rp 0');
                if (myChart) {
                    myChart.destroy();
                }
            }
